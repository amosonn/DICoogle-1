...
public class XMLSupport extends DefaultHandler
{
    ...
    public void startElement( String uri, String localName, String qName, Attributes attribs )
    {
        ...
        else if(localName.equals("destinations"))
        {
            ...
            DebugManager.getInstance().debug("MoveDestinations ---> Found!!!!! ");
        ...
        else if (destinations && localName.equals("dest"))
        {
            ...
            DebugManager.getInstance().debug("Starting Dest inside Destinations");
            MoveDestination tmp = new MoveDestination(this.AETitle, this.IP, this.port)  ;
                s.add(tmp);
                DebugManager.getInstance().debug(tmp.toString());
    ...
    @Override
     public void endElement( String uri, String localName, String qName ) {
        ...
        {
            this.dest = false  ;
            DebugManager.getInstance().debug("Stop Dest inside Destinations");
        }
...
//                 s.setP2P(result);
         ...
         if (isQRConfigs && QREnable)
         {
             ...
             if(tmp.equals("true")){
                 ...
                 DebugManager.getInstance().debug("QueryRetrieve service is enable by default");
             }
             else{
         ...
         {
             String tmp = new String(data, start, length);
             DebugManager.getInstance().debug("qrConfig && SOP class => "+ tmp);
             return;
         }
         if (isQRConfigs && isTransfCap)
         {
            DebugManager.getInstance().debug("qrConfig && transCap");
            return;
         }
        ...
        {
            DebugManager.getInstance().debug("Characters inside destinations");
            return;
...
/*            if (s.isP2P())
            ...
            for (MoveDestination m : moves)
            {
                ...
                DebugManager.getInstance().debug(m.getAETitle());
                DebugManager.getInstance().debug(m.getIpAddrs());
                DebugManager.getInstance().debug(String.valueOf(m.getPort()));
